...
pre_install_docker_compose(){
    ...
    echo "你前端节点信息里面的节点ID"
    read -p "(比如说是3这样子):" ssrpanel_node_id
...
config_docker(){
    ...
    echo "等待加载DOCKER配置文件"
    curl -L https://raw.githubusercontent.com/30665161a/v2/master/docker-compose.yml > docker-compose.yml
...
config_caddy_docker(){
    ...
    echo "开始安装软件包"
    install_dependencies
    curl -L https://raw.githubusercontent.com/30665161a/v2/master/Caddyfile >  Caddyfile
    echo "等待加载DOCKER配置文件"
    curl -L https://raw.githubusercontent.com/30665161a/v2/master/docker-compose.yml > docker-compose.yml
    sed -i "s|node_id:.*|node_id: ${ssrpanel_node_id}|"  ./docker-compose.yml
    sed -i "s|sspanel_url:.*|sspanel_url: '${ssrpanel_url}'|"  ./docker-compose.yml
...
while true
do
echo -e "\033[42;30m 此为由狐狸的脚本汉化版 \033[0m"
echo  ""
echo  "请输入数字选择你要进行的操作："
